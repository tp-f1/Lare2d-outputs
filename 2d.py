...
for i in range(3):
    ...
    data1 = sh.getdata("../DiracData/final/sh/3/" + si + ".sdf")
    data2 = sh.getdata("../DiracData/final/fl/3/" + si + ".sdf")
    data3 = sh.getdata("../DiracData/final/fl/3/" + si + ".sdf")

    T1 = data1.Fluid_Temperature.data * T0
    T1 = T1[250:550,50:400]
    T1 = np.swapaxes(T1,0,1)
    rho1 = data1.Fluid_Rho.data * rho0
    rho1 = rho1[250:550,50:400]
    rho1 = np.swapaxes(rho1,0,1)
    vy1 = data1.Velocity_Vy.data * v0
    vy1 = vy1[250:550,50:400]
    vy1 = np.swapaxes(vy1,0,1)


    T2 = data2.Fluid_Temperature.data * T0
    T2 = T2[250:550,50:400]
    T2 = np.swapaxes(T2,0,1)
    rho2 = data2.Fluid_Rho.data * rho0
    rho2 = rho2[250:550,50:400]
    rho2 = np.swapaxes(rho2,0,1)
    vy2 = data2.Velocity_Vy.data * v0
    vy2 = vy2[250:550, 50:400]
    vy2 = np.swapaxes(vy2,0,1)
    
    T3 = data3.Fluid_Temperature.data * T0
    T3 = T3[250:550,50:400]
    T3 = np.swapaxes(T3,0,1)
    rho3 = data3.Fluid_Rho.data * rho0
    rho3= rho3[250:550,50:400]
    rho3 = np.swapaxes(rho3,0,1)
    vy3 = data3.Velocity_Vy.data * v0
    vy3 = vy3[250:550, 50:400]
    vy3 = np.swapaxes(vy3,0,1)


    x = np.linspace(-15, 15, len(T1[0]))
    y = np.linspace(5, 40, len(T1[1]))
    ext = (-15, 15, 5, 40)
    
    if (i == 200):
        dx = 0.8*L0 / 800
        mfp = 5.5 * 1e7 * T2**2 / (rho2 / (1.2 * 1.67e-27))
        kn = np.sqrt((0.5*(mfp[1:,:]+mfp[0:-1,:])*(T2[1:,:]-T2[0:-1,:]) / (0.5*(T2[1:,:]+T2[0:-1,:])*dx))**2)
        #kn = np.sqrt(0.5*(mfp[:,1:]+mfp[:,0:-1])*(T3[:,1:]-T3[:,0:-1]) / (0.5*T3[:,1:]-T3[:,0:-1]*dx))**2
        out4 = ax4.imshow(kn, extent = ext, origin = 'lower', vmin = 0.01, vmax = 1)
        cbar = fig4.colorbar(out4)


    out1 = ax1[0, indx].imshow(T1, extent = ext, origin = 'lower', norm = 'log', vmin = 5.e5, vmax = 2.e7, cmap = 'magma')
    out1 = ax1[1, indx].imshow(T2, extent = ext, origin = 'lower', norm = 'log', vmin = 5.e5, vmax = 2.e7, cmap = 'magma') 
    out1 = ax1[2, indx].imshow(T3, extent = ext, origin = 'lower', norm = 'log', vmin = 5.e5, vmax = 2.e7, cmap = 'magma') 
    ax1[2, indx].set_xlabel('X (Mm)', fontsize = 16)
    ax1[2, indx].set_xticks([-10,0,10])
    ax1[2, indx].xaxis.set_tick_params(labelsize = 16) 
    ax1[0, indx].set_title("{:.0f}".format((t-0.25)*t0) + " s", fontsize = 18)
    ax1[indx,0].set_ylabel("Y (Mm)", fontsize = 16)
    ax1[indx,0].set_yticks([10,20,30,40])
    ax1[indx,0].yaxis.set_tick_params(labelsize=16)
    
    if (indx == 2):
        rect1 = patches.Rectangle((-3,24.5),6,6, fill = False, color = 'blue')
        rect2 = patches.Rectangle((-5,10),10,23, fill = False, color = 'red', linestyle = "--")
        ax1[2,indx].add_patch(rect1)
        ax1[2,indx].add_patch(rect2)


    out2 = ax2[0,indx].imshow(rho1, extent = ext, origin = 'lower', norm = 'log', vmin = 1.e-13, vmax = 3.e-12, cmap = 'viridis')
    out2 = ax2[1,indx].imshow(rho2, extent = ext, origin = 'lower', norm = 'log', vmin = 1.e-13, vmax = 3.e-12, cmap = 'viridis')
    out2 = ax2[2,indx].imshow(rho3, extent = ext, origin = 'lower', norm = 'log', vmin = 1.e-13, vmax = 3.e-12, cmap = 'viridis')
    ax2[2,indx].set_xlabel('X (Mm)', fontsize = 16)
    ax2[2, indx].set_xticks([-10,0,10])
    ax2[2, indx].xaxis.set_tick_params(labelsize = 16) 
    ax2[0, indx].set_title("{:.0f}".format((t-0.25)*t0) + " s", fontsize = 18)
    ax2[indx,0].set_ylabel("Y (Mm)", fontsize = 16)
    ax2[indx,0].set_yticks([10,20,30,40])
    ax2[indx,0].yaxis.set_tick_params(labelsize=16)

    out3 = ax3[0,indx].imshow(vy1/1.e3, extent = ext, origin = 'lower', vmin = -50, vmax = 50, cmap = 'bwr')
    out3 = ax3[1,indx].imshow(vy2/1.e3, extent = ext, origin = 'lower', vmin = -50, vmax = 50, cmap = 'bwr')
    out3 = ax3[2,indx].imshow(vy3/1.e3, extent = ext, origin = 'lower', vmin = -50, vmax = 50, cmap = 'bwr')
    ax3[2,indx].set_xlabel('X (Mm)', fontsize = 16)
    ax3[2, indx].set_xticks([-10,0,10])
    ax3[2, indx].xaxis.set_tick_params(labelsize = 16) 
    ax3[0, indx].set_title("{:.0f}".format((t-0.25)*t0) + " s", fontsize = 16)
    ax3[indx,0].set_ylabel("Y / Mm", fontsize = 16)
    ax3[indx,0].set_yticks([10,20,30,40])
    ax3[indx,0].yaxis.set_tick_params(labelsize=16)

ax1[0,0].set_ylabel("Y (Mm)", fontsize = 16)
ax1[0,0].set_yticks([10,20,30,40])
ax1[1,0].set_ylabel("Y (Mm)", fontsize = 16)
ax1[1,0].set_yticks([10,20,30,40])
ax1[2,0].set_ylabel("Y (Mm)", fontsize = 16)
ax1[2,0].set_yticks([10,20,30,40])
...
cbar1.set_ticks([1.e6, 3.e6, 1.e7])
cbar1.set_ticklabels(['$10^6$', '3x$10^6$', '$10^7$'])
